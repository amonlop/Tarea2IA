...
{ // Based on the action selection method chosen, it selects an action to execute next
    ...
    {
        int mejor_accion = 0;
        float mejor_valor = Qvalues[x_pos][y_pos][0];

        for(int i = 1; i < 4; i++)
        {
            if(Qvalues[x_pos][y_pos][i] > mejor_valor)
            {
                mejor_valor = Qvalues[x_pos][y_pos][i];
                mejor_accion = i;
            }
        }
        return mejor_accion;
...
void move(int action)
{
    ...
    if(stochastic_actions)
    {
        ...
        if(probabilidad < 0.8) { //(80% de las veces)
            //se continua igual
        } else if(probabilidad < 0.9) { // entre 0.8 y 0.9 (10% de las veces)
            action = (action + 3) % 4; // izquierda
        } else { // entre 0.9 y 1.0 (10% veces)
...
void update_q_prev_state() //Updates the Q value of the previous state
{
    ...
    if(!( ((x_pos==goalx)&&(y_pos==goaly)) ||((environment==1)&&(x_pos==goalx)&&(y_pos==(goaly-1)))||((environment==2)&&(x_pos>0)&&(x_pos<goalx)&&(y_pos==0))) )
    {
        ...
        Qvalues[prev_x_pos][prev_y_pos][action_taken] = Qvalues[prev_x_pos][prev_y_pos][action_taken] + learn_rate * (reward[x_pos][y_pos] + disc_factor * max_q_next - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
...
void update_q_prev_state_sarsa()
{
    ...
    if(!( ((x_pos==goalx)&&(y_pos==goaly)) ||((environment==1)&&(x_pos==goalx)&&(y_pos==(goaly-1)))||((environment==2)&&(x_pos>0)&&(x_pos<goalx)&&(y_pos==0))     ) )
    {
        Qvalues[prev_x_pos][prev_y_pos][action_taken] = Qvalues[prev_x_pos][prev_y_pos][action_taken] + learn_rate * (reward[x_pos][y_pos] + disc_factor * Qvalues[x_pos][y_pos][action_taken2] - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
